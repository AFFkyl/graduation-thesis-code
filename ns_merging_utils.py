...
def _compute_taskvector_cosine_matrix(task_vectors, model) -> Tensor:
    ...
    for param_name, base_param in model.named_parameters():
        ...
        # Build V: [K, D] for this parameter (missing deltas treated as zeros)
        rows = []
        ...
        V = torch.stack(rows, dim=0)  # [K, D] float32 on CPU
        gram += (V @ V.t()).to(dtype=torch.float64)  # accumulate exact dot products
...
def _apply_ties_to_parameter_across_tasks(
    ...
    c_p = (disjoint_selected != 0).sum(dim=0).float()
    c_p_safe = torch.clamp(c_p, min=1.0)

    for task_idx, task_vector in enumerate(task_vectors):
        new_flat_delta = disjoint_selected[task_idx].to(ref_dtype)
        new_flat_delta = new_flat_delta / c_p_safe.to(dtype=ref_dtype)
...
def _project_weight_to_right_nullspace(weight, activations, rcond=0):
    r"""
    Project weight onto the right nullspace of activations (i.e., enforce W X^T = 0)
    using the Moore–Penrose pseudoinverse:

        W_proj = W - W X^T (X X^T)^\dagger X = W (I - P),   P = X^T (X X^T)^\dagger X

    Args:
        weight: Tensor, shape (m, d)
        activations: Tensor, shape (n, d)  (each row is an activation sample)
    ...
    # A = X X^T  (n x n), possibly singular => use pseudoinverse
    gram = activations_fp32.mm(activations_fp32.t())
    gram_pinv = torch.linalg.pinv(gram, rcond=rcond)  # (X X^T)^\dagger

    # correction = W X^T (X X^T)^\dagger X
    ...
    # Rescale projected W to match ||W||_F (not part of projection optimality)
...
def ns_merging(
    ...
    num_ns_cycles = getattr(args, "ns_cycles", 8)  # 通常 R = K，即循环次数等于任务数
    ...
    bias_reset_ratio = getattr(args, "ties_reset_thresh_bias", 0.4)  # Ties原文，保留40%
